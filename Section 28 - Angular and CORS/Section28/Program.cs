...
{
    //This can be now use in EnableCORS Decorators inside controllers
    options.AddPolicy("4100Client", policyBuilder =>
    {
        policyBuilder.WithOrigins(builder.Configuration.GetSection("Allow-Origins1").Get<string[]>());
        policyBuilder.WithHeaders("Field-Key", "Authorization", "origin", "content-type");
        policyBuilder.WithMethods("PUT", "DELETE");
    });

    options.AddDefaultPolicy(policyBuilder =>
    {
        policyBuilder.WithOrigins(builder.Configuration.GetSection("Allow-Origins").Get<string[]>());
        policyBuilder.WithHeaders("Field-Key","Authorization","origin","content-type");
